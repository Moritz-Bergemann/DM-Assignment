...
def _prep_dataset(  df:pd.DataFrame, 
    ...
    printv(f"Converting columns with >={cat_binning_threshold} unique values to categorical: {lt_cat_threshold_cols}")
    ...
    printv(f"Removing columns with >={0.8} missing values: {list(cols_missing_values_80.index)}")
    ...
    for col in cat_cols:
        ...
        if cols_missing_values[col] > 0:
            ...
            ## In remaining cat. columns with >=0.05 missing values, make 'missing' a category
                ...
                printv(df[col].unique())
                printv(type(df[col].cat))
                printv(df[col].cat.categories)
    ...
    printv(f"Dropping highly monotonous categorical cols: {high_monotony_cols}")
    ...
    printv(f"Final coumns (df_train.columns): {list(df_train.columns)}")
...
def get_prepped_dataset(bins=None, cat_binning_threshold=30, bin_type='width',
    ...
    return _prep_dataset(df, bins=bins, cat_binning_threshold=cat_binning_threshold, bin_type=bin_type,
                    ...
                    cols_to_drop=cols_to_drop, verbose=verbose)
